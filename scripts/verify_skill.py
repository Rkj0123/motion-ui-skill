...
def main() -> int:
    ...
    expected_categories = {"motion": 52, "agents": 17, "blocks": 26}
    actual_categories = {
        category: sum(item.get("category") == category for item in components.values())
        for category in expected_categories
    }
    assert len(components) == 95, len(components)
    ...
    for name in ("README.md", "SKILL.md"):
        ...
        assert len(rows) == 95, (name, len(rows))
    ...
    print("Motion UI skill integrity: PASS (95 components, 52/17/26 categories)")
    return 0
