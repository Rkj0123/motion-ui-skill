...
description: >-
  ...
  and Tailwind CSS. Includes 82 interactive components across 3 categories: Motion Primitives (buttons,
  inputs, tabs, modals, bottom sheets, sliders, accordions, dock, carousels, loaders), AI Agent Surfaces
  (message bubbles, scrollers, prompt inputs, streaming responses, diff viewers, approval cards, activity streams),
  and Blocks & Widgets (dynamic island, command palette, cross-chain swap, prediction market, availability scheduler).
  100% offline with complete source code, shared utilities, animation physics tokens, and installation procedures.
---

# Motion UI Skill

> Complete offline repository of **82 production-ready animated UI components** for React and Next.js, built with **Motion** (`motion/react` / `framer-motion`) and **Tailwind CSS**.

This skill is completely **self-contained and offline-first**. All source code, shared animation utilities, hooks, physics tokens, and component guides exist directly in this repository. **No external internet calls, API requests, or web searches are required.**
...
1. **Locate the Component**: Find the matching component from the 82 components listed in the catalog below or in [`catalog.json`](./catalog.json).
...
4. **Install NPM Dependencies**:
   - Inform the user or run: `npm install motion clsx tailwind-merge` + any specific component dependency (e.g. `lucide-react`, `shiki`, `@tanstack/react-virtual`).
...
### Category 1: Motion Components (42 Components)
...
### Category 3: Blocks & Composed Widgets (23 Components)
Complex composable animated UI widgets: dynamic island, command palette, multi-chain swap, prediction market, availability scheduler, and folder views.
...
For in-depth animation principles, timing decisions, and accessibility rules, read:
