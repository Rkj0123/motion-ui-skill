...
const VT_CSS = `
html[data-motion-vt="rect"]::view-transition-old(root) {
  animation: none;
  mix-blend-mode: normal;
}
html[data-motion-vt="rect"]::view-transition-new(root) {
  mix-blend-mode: normal;
  animation: motion-rect-reveal 400ms ease-out;
}
html[data-motion-vt="circle"]::view-transition-old(root),
html[data-motion-vt="circle-blur"]::view-transition-old(root) {
  animation: none;
  mix-blend-mode: normal;
}
html[data-motion-vt="circle"]::view-transition-new(root) {
  mix-blend-mode: normal;
  animation: motion-circle-reveal 700ms cubic-bezier(0.4, 0, 0.2, 1);
}
html[data-motion-vt="circle-blur"]::view-transition-new(root) {
  mix-blend-mode: normal;
  animation: motion-circle-blur-reveal 700ms cubic-bezier(0.4, 0, 0.2, 1);
}
html[data-motion-vt="blinds"]::view-transition-old(root) {
  animation: none;
  mix-blend-mode: normal;
}
/* Slats: a masked band widens inside every 72px tile, so the new theme opens
   ...
   step. */
@property --motion-vt-slat {
  syntax: "<length>";
  inherits: false;
  initial-value: 72px;
}
html[data-motion-vt="blinds"]::view-transition-new(root) {
  mix-blend-mode: normal;
  mask-image: linear-gradient(
    90deg,
    #000 0 var(--motion-vt-slat),
    transparent calc(var(--motion-vt-slat) + 20px)
  );
  mask-size: 72px 100%;
  mask-repeat: repeat;
  animation: motion-blinds-reveal 700ms ${EASE_OUT_CSS};
}
@keyframes motion-rect-reveal {
  from { clip-path: var(--motion-vt-from, inset(100% 0 0 0)); }
  to   { clip-path: inset(0 0 0 0); }
}
@keyframes motion-circle-reveal {
  from { clip-path: circle(0% at var(--motion-vt-origin, 50% 100%)); }
  to   { clip-path: circle(150% at var(--motion-vt-origin, 50% 100%)); }
}
@keyframes motion-circle-blur-reveal {
  from { clip-path: circle(0% at var(--motion-vt-origin, 50% 100%)); filter: blur(8px); }
  to   { clip-path: circle(150% at var(--motion-vt-origin, 50% 100%)); filter: blur(0px); }
}
@keyframes motion-blinds-reveal {
  from { --motion-vt-slat: -20px; }
  to   { --motion-vt-slat: 72px; }
}
`;
...
export function useThemeToggle({
  ...
  const toggle = () => {
    ...
    if (variant === "rectangle") {
      root.style.setProperty("--motion-vt-from", RECT_FROM[start]);
      root.dataset.motionVt = "rect";
    } else if (variant === "blinds") {
      // Slats sweep the whole viewport; there is no origin point to set.
      root.dataset.motionVt = "blinds";
    } else {
      root.style.setProperty("--motion-vt-origin", CIRCLE_ORIGIN[start]);
      root.dataset.motionVt = variant;
    }
